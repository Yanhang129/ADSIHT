...
result <- sapply(1:100, function(i){
  ...
  lasso1 <- c(f(true, lasso), detect.group(beta1, J, k), sqrt(sum(sum((beta1-beta)^2)+intercept^2)/sum(beta^2)), t)
  round(lasso1, 3)
  ...
  beta1 <- coef[-1]
  intercept <- coef[1]
  gbridge1 <- c(f(true, gbridge), detect.group(beta1, J, k), sqrt(sum(sum((beta1-beta)^2)+intercept^2)/sum(beta^2)), t)
  round(gbridge1, 3)
  ...
  beta1 <- coef[-1]
  intercept <- coef[1]
  explasso1 <- c(f(true, explasso), detect.group(beta1, J, k), sqrt(sum(sum((beta1-beta)^2)+intercept^2)/sum(beta^2)), t)
  round(explasso1, 3)
  ...
  beta1 <- coef[-1]
  intercept <- coef[1]
  MCP1 <- c(f(true, MCP), detect.group(beta1, J, k), sqrt(sum(sum((beta1-beta)^2)+intercept^2)/sum(beta^2)), t)
  round(MCP1, 3)

  t <- as.numeric(system.time(fit <- DSIHT(x, y, group = group, L = 5, rho = 0.95, method = "fast2", ic.type = "ebic", ic.coef2 = 0.5))[3])
  l <- which.min(fit$ic)
  coef <- c(fit$intercept[l], fit$beta[, l])
  best.group <- which(fit$beta[, l] != 0)
  dsiht <- rep(0, p)
  dsiht[best.group] <- 1
  dsiht1 <- c(f(true, dsiht), detect.group(coef[-1], J, k), sqrt(sum(sum((coef[-1]-beta)^2)+coef[1]^2)/sum(beta^2)), t)
  round(dsiht1, 3)

  return(c(lasso1, gbridge1, explasso1, MCP1, dsiht1))
})
res[[snr]] <- matrix(round(apply(result, 1, mean), 2), ncol = 5)
}
...
save(res, file = "C:/Users/Yanhang/Desktop/code/result/res_snr_normal_rerr.rda")
