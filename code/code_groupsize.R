...
gen.coef <- function(s, k, Tn, J, i, signal){
  ...
  # temp1 <- runif(s*Tn, signal, 5*signal)*(2*rbinom(s*Tn, 1, 0.5)-1)
  for (i in 1:Tn) {
    temp2 <- rep(0, k)
...
sgl <- function(x, y, group){
  fit <- cv.sparsegl(x, y, group, nfolds = 5, eps = 1e-6)
  coeff <- coef(fit, s = "lambda.1se")
  beta <- coeff[-1]
  inter <- coeff[1]
...
grp <- function(x, y, group, penalty){
  fit <- grpreg(x, y, group = group, penalty = penalty, lambda.min = 0.05, eps = 1e-8, dfmax = 200)
...
result <- sapply(1:100, function(i){
  ...
  return(c(lasso1, explasso1, MCP1, dsiht1))
})
